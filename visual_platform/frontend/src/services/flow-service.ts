...
export const flowService = {
  ...
  async duplicateFlow(id: number, newName?: string): Promise<Flow> {
    const url = `${API_BASE_URL}/api/flows/${id}/duplicate${newName ? `?new_name=${encodeURIComponent(newName)}` : ''}`;
  ...
  async createDefaultFlow(nodes: any, edges: any, viewport?: any): Promise<Flow> {
    return this.createFlow({
      name: 'My First CAIAO Flow',
      description: 'Welcome to CAIAO Visual Platform! Start orchestrating your Server pipeline here.',
      nodes,
      edges,
      viewport,
    });
