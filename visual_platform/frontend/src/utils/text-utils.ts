import { extractBaseServerKey } from '@/data/node-mappings';
 ...
 */
export function createServerDisplayNames(serverIds: string[]): Map<string, string> {
  // Extract base server keys and count occurrences
  const baseServerCounts = new Map<string, number>();
  const baseServerKeys = serverIds.map(id => extractBaseServerKey(id));

  baseServerKeys.forEach(baseKey => {
    baseServerCounts.set(baseKey, (baseServerCounts.get(baseKey) || 0) + 1);
  });

  // Create display names with numbering for duplicates
  const baseServerCounters = new Map<string, number>();
  const displayNames = new Map<string, string>();

  serverIds.forEach(serverId => {
    const baseKey = extractBaseServerKey(serverId);
    const count = baseServerCounts.get(baseKey) || 1;

    // Convert snake_case to readable format
    const readableName = baseKey.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase());

    if (count > 1) {
      const currentCounter = (baseServerCounters.get(baseKey) || 0) + 1;
      baseServerCounters.set(baseKey, currentCounter);
      displayNames.set(serverId, `${readableName} #${currentCounter}`);
    } else {
      displayNames.set(serverId, readableName);
    }
  });
